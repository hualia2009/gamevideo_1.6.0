...
import com.leslie.gamevideo.AdInfo;
import com.leslie.gamevideo.AppConnect;
import com.leslie.gamevideo.R;
import com.leslie.gamevideo.SDKUtils;
...
public class AppWall extends Activity {
	...
	private View getContentView(final Context context){
		...
		layout.setBackgroundColor(Color.parseColor("#ff1e1f21"));
		
		try {
			layout.setOrientation(LinearLayout.VERTICAL);
			// 标题布局
			RelativeLayout title_layout = new RelativeLayout(context);
			title_layout = (RelativeLayout) LayoutInflater.from(context).inflate(R.layout.titlebar, null);
			if(displaySize == 320){
			...
			title_layout.setLayoutParams(new LinearLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.WRAP_CONTENT));
...
//			title_layout.addView(button, params2);
			...
			listView.setBackgroundColor(Color.parseColor("#1e1f21"));
//			listView.setBackgroundDrawable(getResources().getDrawable(R.drawable.group_bg));
...
//				new int[] {Color.parseColor("#cccccc"), Color.parseColor("#ffffff"), Color.parseColor("#cccccc")}); 
			Drawable divider_line = getResources().getDrawable(R.drawable.list_divider_line);
			listView.setDivider(divider_line);
	...
	private void initOnclickerListener() {
		imgbtnHistory.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent = new Intent(AppWall.this, History.class);
